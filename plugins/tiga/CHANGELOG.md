...
- 统计页「今日统计」卡片新增「下次提醒时间」展示，随定时器触发实时推进；暂停时显示「已暂停」

### 修复

- 修复首次安装后进入插件「状态显示已开启但不提醒」的问题：插件加载时若存储中还没有配置，自动写入一份默认配置并启动定时器，保证 UI 显示状态与实际运行状态一致（此前 UI 显示的是前端默认值，定时器实际从未启动，需手动开关一次才生效）
