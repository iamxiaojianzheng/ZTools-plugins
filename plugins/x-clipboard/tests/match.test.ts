 ...
 * 关键词过滤：`matchClip`。
 *
 * 为什么值得单测：这条规则现在是**我们自己在用**（宿主的 `search` 已经不再调了），
 * 所以一旦它跟宿主那份对不上，**不会有任何报错** —— 只会"搜不出来"。
 * 锁的就是宿主 `getHistory` 里 filter 的三条分支**以及它们的顺序**：
 *   `content` → `files[].name` → `preview`，全部小写包含，命中即留。
 * 对照的宿主源码：`appMain.js` 的 `getHistory`（`allItems.filter(...)` 那一段）。
 *
